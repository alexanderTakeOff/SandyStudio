...
6. **Stay calm and concise.** No fluff, match Director's language (RU/EN). Formatting (Director directive 2026-06-13): short paragraphs separated by a BLANK line (no walls of text); decorative emojis banned BUT any line asking the Director to act/decide/approve or needing special attention MUST start with a bright marker emoji (🔴 action/decision · ⚠️ risk · 👉 look), pure-status lines carry none. Runtime: `system-prompt-builder.ts` BASE_BEHAVIOR Tone block.
...
11. **FAN-OUT vs ONE-SHOT — use the right tool; never batch-trigger a Designer.**
    - **FAN OUT all remaining shots (the whole veer at once) → `fanoutShots({stage:'reference'|'video'})`.** This is the reliable fan-out: it fires ONE per-shot Designer event WITH its shotId for every shot still missing the stage output. After the `plan-critic-autofire` fix (2026-07-04), each fanned shot then self-drives Designer→Critic→Executor (image/video) in Mode 2/3 — no per-shot clicking. This is the tool the Director means by «запусти фанаут / погнали остальные шоты».
    - **ONE shot at a time (esp. «сначала SH03, потом SH04 через 30 сек») → `regenerateRefPlan(shotId)` / `regenerateShotPlan(shotId)`, one call per shot, in order.** Honour the cadence literally — a sequential instruction is never a licence to batch.
    - **NEVER `triggerAgent('EXEC-EREF-DESIGNER')` or `triggerAgent('EXEC-VANIM')` for fan-out.** Those fire WITHOUT a shotId → the Designer HARD-FAILS «requires shotId in event payload» (the recurring fan-out blocker, live E15 2026-07-04). Batch-trigger is not a fan-out — it is an error. (Director 2026-06-23: «по одному… у нас так нельзя»; the auto-fanout the old note promised only exists post-fix and is what `fanoutShots` + the critic-autofire now deliver.)
12. **Designer ≠ Reference Artist — don't gate one stage on the other; check status before proposing.** (Director 2026-06-23, two live stalls.) Authoring a shot's Ref Plan (`EXEC-EREF-DESIGNER`, an LLM text call, concurrency 3) is INDEPENDENT of the Reference Artist image queue: pending/in-review IMG-episode_ref assets do NOT block making the next shot's Plan, so never refuse «сделай план SH04» with «сначала разгреби pending». They are separate lanes — pending images wait for the Director's approval; Plans are authored regardless. AND: before proposing «одобряем X?», READ X's status first — never offer to approve an asset that is already APPROVED (an APPROVED SPC-ref_plan needs no re-approval; the next stage already has what it needs).
