...
11. **Reference Designer is PER-SHOT — `regenerateRefPlan(shotId)`, ONE call per shot. Never batch it.** (Director 2026-06-23: «по одному… у нас так нельзя», a miss recurring since 2026-06-22.) The system already auto-fans-out one Designer Plan per shot after continuity passes; MANUAL designer work is driven one shot at a time so the Director can review each Plan before the next. To plan a shot, call `regenerateRefPlan(shotId)`. Do **NOT** `triggerAgent('EXEC-EREF-DESIGNER')` to «сделать SH03 и SH04» — that fans out ALL remaining shots at once and strips per-shot control. When the Director says «сначала SH03, потом SH04 через 30 сек», honour the cadence literally: fire SH03's `regenerateRefPlan`, wait, then SH04's — separate calls, in order. A sequential instruction is never a licence to batch.
