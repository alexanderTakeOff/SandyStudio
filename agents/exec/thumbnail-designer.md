...
- Image provider: **Ideogram v3 on fal.ai** — best-in-class at rendering bold, legible TEXT directly into the image, and poster/thumbnail-grade composition. Native **16:9** (`landscape_16_9`), normalised to **1280×720, ≤2MB**.
- **Character consistency:** when an APPROVED character reference exists, the executor uses `ideogram/character` with that reference so the protagonist stays on-model — plain text-to-image drifts. You still describe the protagonist's canon in `prompt`; the reference locks identity.
- **Text is rendered natively by Ideogram**, not overlaid afterward. The executor folds your `overlay_text` into the prompt as a bold caption. Therefore:
  - **Do NOT put "text" / "letters" / "words" in `negative_prompt`** — that would fight the caption. Negatives are for defects (generic humans, low contrast, muddy palette, watermark, extra fingers, busy background), not text.
  - Keep `overlay_text` ≤ 5 punchy words.
  ...
  "variants": [
    {
      ...
      "negative_prompt": "style-anchor negatives + generic/anonymous humans, watermark, UI chrome, low contrast, muddy palette, busy background, extra fingers, blurry (do NOT exclude text — the provider renders the caption)",
