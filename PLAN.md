...
```
Date:   2026-08-29 (парадигма ВЛИТА в master; Seedance 2.5; час кэша Полины).
Status: **`paradigm/direct-mind` → master fast-forward, 40 коммитов (`be1ffae9`).**
  Живые порты: master :3000/:8288 (лежит), парадигма **:3001/:8388** — правда в
  `webapp/.env.local` и `start-stack-nx.cmd`; прежняя строка ЗДЕСЬ писала `:8289`
  и стоила сессии путаницы. Третья копия номера порта — ложь, ждущая своего часа.
  Час кэша промптов вернулся в подписочный спавн Полины (`ENABLE_PROMPT_CACHING_1H`
  в `sanitizeSubscriptionEnv`): допущение «эпизод целиком в ОДНОЙ горячей сессии»
  жило замером в плане Ф6, а не в коде — замер задержки дал 264 с медиана, из них
  53 с до первого действия. Видео переведено на **Seedance 2.5** (4–30 с одним
  проходом); потолок длительности читается из модели, а не из константы 15.
  S22-E03 — первый эпизод на живом заказчике: канон лица и клон голоса сняты,
  ролик №2 «ДЕЛО» в работе, пакет из пяти.
Next:   **спайк 30-секундного прохода на 2.5** — ростовой кадр по постановке E01,
  героем Осинцев. Меряем не «красиво ли», а держится ли ЛИЦО тридцать секунд:
  дрейф растёт со временем и у живого заказчика наказуем, в отличие от рисованного
  героя. Затем звук на ролик №2 и остальные четыре ролика пакета.
