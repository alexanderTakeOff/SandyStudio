...
```
Date:   2026-07-31 (Канон-минимум «Глубины» ЗАКРЫТ + четыре разрыва авторинга — Тео, ===5===; ночь).
Status: SS-S20 несёт 6 LOCKED канон-ассетов, шаг §1-10 закрыт. По коду — четыре разрыва,
  все найдены прогоном и закрыты: D8 писатель канона не умел ПРАВКУ (у `enrichBible`
  появился `notes`: текущая статья уходит неприкосновенной БАЗОЙ, правки дельтой);
  D9 нельзя было рисовать поверх своего же кадра (запись может объявить САМУ СЕБЯ на
  строке `Refs:` — компоновка переживает правку только приходя пикселями); D10 ревизия
  ЗАТИРАЛА историю версий (общий `nextVersionFor`, вынесен из `regenerate-image`);
  D11 секция `style` держала два претендента на роль якоря, а `bible.styles[0]` брался
  из запроса БЕЗ `ORDER BY` — лотерея на каждом запуске, живёт на S14 с 12 мая. Лечится
  не сортировкой, а именованием: `style` = ОДИН документ направления на серию, всё
  прикладываемое живёт в object/location/character (Директор, q6). Плюс закон плиты
  в `agents/exec/bible_author.md`: писатель дважды за час вносил в канон-плиту то, что
  решает ЭПИЗОД. Полина исполнила 4 правки подряд с первого сообщения (до этого вставала
  трижды). tsc · 1655/1655 · replay-pilot 30/30. Задеплоено. Push `dfda26a1`.
Next:   шаг §1-11 — Mode 1 → `createEpisode` для S20-E01 → конвейер. Материал брифа готов:
  раскадровка (8 кадров) + ФИНАЛЬНЫЙ КАДР со слов Директора в `FILMS/Deep/episode_ideas.md`,
  ТЗ в `pilot_client_brief.md`. Позиция прогона — `docs/plans/series-bootstrap-run-notes.md` §0.
