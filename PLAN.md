...
Status: Директор: «вкладка бюджет работала на первых эпизодах, потом перестала».
  ...
  журнал перевалил за 1000, страница замерла: старые эпизоды показывали траты (их
  строки первые по порядку вставки), все новые = $0. Тихо, без падений и логов.
  (`/api/factory` уже носил рукописную пагинацию из-за того же капа — это и
  подтвердило диагноз.)
  ЧТЕНИЕ: `lib/api/paged-select.ts` (общий, вынут из factory, с anti-spin гардом) ·
  `/api/budget` пагинирует оба скана + отдаёт counts (будущее усечение ВИДНО) ·
  `lib/budget-summary.ts` — чистая тестируемая математика: тотал эпизода = ПОСТАТЕЙНАЯ
  сумма (`budget_log`), `budget_spent` показан отдельно как `reserved`, разрыв = `drift`
  (правило синхронно с /api/factory — две страницы больше не могут разойтись в цифре).
  Burn считался по журналу резерваций (без Полины), а тотал — с ней; «последние 5»
  были в произвольном порядке БД. Исправлено. Перерасход больше не обрезается в $0.
  Предохранитель трат Полины тоже читал без пагинации — ошибка в ОПАСНУЮ сторону
  (траты после 1000-й строки невидимы ⇒ кап не мог сработать).
  5 ДЫР, где деньги не писались ВООБЩЕ: (1) EXEC-EREF пилот+фан-аут — функция обходит
  фабрику и вместе с ней шаг record-cost; весь счёт за референсы (генерация + vision-
  критики EREF-check и on-model) уходил только в метаданные activity_events; (2) 4K-
  апскейл — платный fal на КАЖДОМ APPROVE референса, цена терялась в выхлопе Inngest;
  (3) Visual Critic — usage считывался и выбрасывался (~50 референсов + 10 кадров на
  видео-шот за прогон); (4) Style Guardian — cost_usd считался и игнорировался всеми
  тремя вызывающими; (5) обогащение брифа при создании эпизода. Полина писалась
  корректно — не потеряно ничего.
  ЧЕСТНОСТЬ: сумма частей = заголовок (расход без эпизода → блок `unattributed`) ·
  панель «Что эта страница не видит» ($0-мок, эпизоды без постатейных строк, drift,
  модели без цены) · экран ошибки (раньше при сбое API вечно пустой заголовок).
  Цены НЕ выдуманы: модель без записи в таблице тихо биллится по тарифу Sonnet, страница
  ИМЕНУЕТ такие модели. ЖИВЫЕ, ждут реальных тарифов: `gemini-2.5-pro`, `gemini-3.x`,
  `gpt-5.6*`, `claude-sonnet-5` → сверить с прайс-страницами вендоров.
  Решения Директора: q1n бэкфилл истории НЕ делаем (деньги до этого коммита в budget_log
  никогда не писались, из него не достаются) · q3y музыка остаётся мок-$0, но показана
  как ДЫРА, а не как экономия.
Verify: tsc clean · 1497/1498 тестов (+19 новых) · replay-pilot 30/30 · next build ок.
  Единственный упавший — ПРЕДСУЩЕСТВУЮЩИЙ windows-путь в storage-probe, падает и на
  чистом master под Linux (push-хук пройден его штатным `# no-verify` только из-за него).
Next:   Смоук на живой БД: открыть /budget → сверить тотал с суммой частей, убедиться что
