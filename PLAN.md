...
```
Date:   2026-07-29 (Проход правок по аудиту E33 — Тео, ===5===; ночь + утро).
Status: 18 из 20 пунктов реестра `docs/plans/e33-pipeline-audit-fix-registry.md` закрыты
  (мерж `7e18022e`). Корень многомесячного «вторая генерация хуже первой»: ретрай выбрасывал
  утверждённый план и подставлял ≤2000-символьный пересказ критика — теперь план неизменяем,
  замечания приклеиваются через существующий `mergeRevisionNote`; три канала перезаписи и
  ЧЕТЫРЕ обрезки удалены. Порог keep-bar 85→70 (был недостижим: максимум за 63 генерации = 77).
  Тот же принцип разнесён по видео-ветке: ручной ре-рендер читает план, а не легаси-шаблон,
  и берёт провайдера/тир/сид/длительность оттуда же. Заработали проверки, не срабатывавшие
  НИ РАЗУ: REGENERATE держит ассет (+ явный обход со следом), пропуск проверки не штампует
  100/100/100, критик читаемости видит разбираемый шот, а не первые 4000 символов доски.
  Вне реестра: пустой `applies_when` = WILDCARD (активация жанрового движка была жребием
  Haiku, HALT критика читаемости недостижим); две поверхности Inbox не читали `res.ok` —
  отказы молча проглатывались. Канон: тело Сэнди было ОБОРВАНО на 5191 символе — восстановлено
  + физика крышек и запечатанности (ноги остаются); Baby-Timer снова прозрачное стекло;
  плоские формулировки убраны из 19 карточек (19→0); кроватка стала каноном; у спальни
  появились световые состояния. tsc чисто · 1599/1599 · replay-pilot 30/30.
  ВИДЕО-ФАНАУТ E33 ПРОШЁЛ: 8 из 9 шотов с видео, SH02 после ручного пуска Директора,
  эпизод сшит. Живой прогон вскрыл ещё пять дефектов одной формы «то, что должно было
  освободить место, продолжало его занимать; то, что должно было сказать, молчало» —
  все закрыты в `a3eafd74`: видео-кэп считал ОТКЛОНЁННЫЕ рендеры (отказ съедал квоту на
  собственную замену); у реконсилятора не было владельца денежной ячейки в REVISION;
  галочка аппрува кончалась пустым `catch {}`; таймлайн и дровер расходились (три копии
  резолвера URL + дисковый кэш миниатюр отдавал подменённые байты вечно); модалки писаны
  для машины. Плюс: упавший ран теперь закрывает свою строку `jobs` (фантомная «идущая
  генерация» висела 2.5 часа), а `start-stack.ps1` больше не стирает лог той аварии, из-за
  которой его перезапускают. Найдено крупнее пола Сэнди: `bible-loader.ts:75` кормит промпт
  полем `description`, где у Сэнди лежал ЗАГОЛОВОК в 41 символ — текстового якоря персонажа
  в видео не было вообще; записаны настоящие описания Сэнди и Baby-Timer с полом.
Next:   ⚠️ РЕБИЛД НЕ СДЕЛАН — на проде код БЕЗ всего перечисленного (решение Директора).
  За Директором: три пластины канона (стиль-канон всё ещё flat-2D → критик штрафует
  корректные 2.5D/3D кадры), выбор стиля пластины (вариант A + карандашная обводка),
  раскол геометрии SH01, SH07 с четырьмя руками. Долг: репер и `closeOpenJobsForRun` —
  почти дубликаты, схлопнуть. Фаза 1 (прогон 3–4 эпизодов) НЕ начата.
Mode:   ===5=== (Директор, 2026-07-29).
```
...
Status: `webapp/scripts/hog-daily.ps1` в git; задача `\SandyStudio\SandyStudio_HoG_Daily`
  ...
  без BOM как ANSI.
  (6) ГЛАВНОЕ, найдено A/B-пробой в реальный `claude -p`: PS 5.1 пишет в пайп нативному exe
  как ASCII → мозг ВСЕ прогоны получал промпт с кириллицей, стёртой в `?` («??????? ????????
  ?????? … 42» — headless-сессия сама это написала), и держался на английских словах,
  структуре и файлах репо. Фикс: `$OutputEncoding` + `[Console]::OutputEncoding` = UTF-8
  без BOM, промпт читается `[IO.File]::ReadAllText(...,UTF8)` (`Get-Content -Raw` под 5.1
  читал UTF-8-без-BOM как ANSI — второй слой той же порчи).
  Доставка (q17a): выжимка текстом + report.html вложением ntfy. Вложение живёт ТОЛЬКО 3ч
  (проверено `attachment.expires`), текст 12ч, сам отчёт — в git. daily-prompt.md шаги 4–6
  переписаны: Artifact выкинут, добавлено требование САМОДОСТАТОЧНОГО html (открывают в
  браузере телефона), доставка снята с мозга. Задача осталась на `powershell.exe` 5.1
  осознанно: pwsh 7.6.4 — Store-сборка, реальный путь пиньён к версии (`…7.6.4.0…`), алиас —
  0-байтный AppExecutionAlias → задача умерла бы молча на обновлении pwsh.
Verify: tsc clean (webapp, exit 0). Живьём: ntfy-текст с кириллицей доставлен, вложение
  report.html отдано (30512 б, text/html), A/B-проба промпта до/после фикса.
  Прогон 14:15 с ЦЕЛЫМ промптом (5 мин, доставка проверена в топике) сразу дал то, чего
  слепой не дал: мозг понял, что это второй замер за сутки, не стал пересказывать числа и
  нашёл дыру в фундаменте — снапшотов за 22/23/24 июл НЕТ, а имя `<дата>.json` затирало
  утренний замер вечерним. Проверено: задач для снапшотов на этой машине не было ни одной
  (старой `SandyStudio_YT_Snapshot` здесь нет), т.е. ряда просто не существовало, и любая
  «суточная» дельта в отчётах 07-25 — это дельта за 4 дня. Фикс (q19): имя снапшота несёт
  UTC-время `<дата>T<HHMM>Z.json` → перезапись невозможна ПО КОНСТРУКЦИИ, без новой опции
  и без проверки-гварда. Утренний замер 09:36Z восстановлен из `a3d206b8`; за 07-25 теперь
  три точки (0936/1016/1033Z). daily-prompt.md: читать самый поздний за сегодня + прямо
  говорить о пропущенных днях, не подавая многосуточную дельту как суточную.
Next:   прогон 26 июл 09:00 — первая точка НАСТОЯЩЕГО суточного ряда. Сверить его отчёт с
  сегодняшним. Вложение ntfy живёт 3ч — если читаешь пуш днём, тап-ссылка мертва.
```
