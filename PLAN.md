...
| 18 | Prod Assistant TTS quality — Director confirmed 2026-05-08 smoke: голос звучит "как больной робот". Web Speech SpeechSynthesis на Windows использует системные голоса (ru-RU = Pavel/Irina), качество неровное. Upgrade path: ElevenLabs API или OpenAI TTS API (~$0.015/1K chars). Решение принимаем после второго использования — может, и не критично. | UX |
...
| LT-10 | **Scalable timeline for 60+ shots** — current Videomatic shot strip renders all cells at once. At 60+ shots cells become too narrow to label or click. Options: virtualised horizontal scroll, mini-map zoom, or chapter-grouping by SC (scene). Trigger: when first episode crosses ~30 shots. | PLANNED — backlog | Director directive 2026-05-07 |
| LT-11 | **Episode page noise cleanup** — page contains pillbar + Videomatic + activity feed + cards stacked vertically. Re-organise into tabs / collapsible sections / progressive disclosure. Coordinate with `specs/system/uiux.md`. | PLANNED — backlog | Director directive 2026-05-07 |
