...
```
Date:   2026-07-28 (Топик-досье: память долгоживущих тем — Тео, ===5===; день).
Status: Заведён слой состояния тем `docs/topics/` — 5 досье (дистрибуция и аудитория ·
  продажа студии · второй жанр · метрики автономности · роли и гейты), формат
  РЕШЕНО/ОПРОВЕРГНУТО/ОТКРЫТО+чем закрывается/КУДА СМОТРЕТЬ. Разборы в docs/analysis
  остаются доказательствами, в контекст грузится только строка состояния каждой темы.
  Оглавление ВЫВОДИТСЯ хуком session-anchors.cjs из директории — индекса-файла нет
  намеренно (рукописный индекс надо помнить, а помнимое гниёт: так MEMORY.md дорос до
  152 инжектируемых строк). Потолок 80 строк блокируется topic-dossier-guard.cjs и
  служит механизмом удаления. Разбор обязан обновить своё досье — CLAUDE.md §9.5 + §7.7.
  Плата внесена: 53 строки бэклога вынесены из MEMORY.md в backlog-index.md (153→100).
  Механизм сразу отработал на себе: c8a0fe28 опроверг два утверждения часового досье —
  фаза 0 (приборы) НЕ блокер, разбивка уже в `activity_events.event_type`; Полина даёт
  0–2 события/эпизод, значит разрыв в оценке касаний объясняет не она; настоящее число
  автономности — 8–71 ручной пуск на эпизод, аппрувов до 90.
Next:   Фаза 1 — прогон 3–4 эпизодов Sandy на замороженном стеке (Полина off, Тео не
  трогает пайплайн, цикл объявлен списком заранее → всё сверх списка = «вне цикла»
  диффом). НЕ заблокирована приборами. Досье-механизм: у остановки всё ещё нет
  артефакта (дрейф-нота) — `docs/topics/roles-and-gates.md` → ОТКРЫТО.
Mode:   ===5=== (Директор, 2026-07-28, q1/q2y/q3y).
```
