...
```
Date:   2026-08-06 (НОВАЯ ПАРАДИГМА — ветка поднята, код переведён; день).
Status: **ВЕТКА `paradigm/direct-mind` В ДЕРЕВЕ `C:\SandyStudio-nx` — 18 коммитов.**
  master цел и работает на :3000/:8288; парадигма на :3001/:8289
  (`start-stack-nx.cmd`; порты стали параметрами, остановка бьёт только по своему
  дереву — раньше запуск второго дерева гасил первое). Обе ветки на origin.
  НА MASTER: кэш промптов включён там, где тратятся деньги — нативный путь с
  `cache_control` был построен и доказан смоуком (−86%), но подключён к ОДНОМУ
  роуту из трёх, и не к интерактивному чату Директора (192 вызова, $34,21) и не к
  ambient-реакции ($13,17): оба шли через OpenAI-совместимую поверхность, которая
  молча выбрасывает `cache_control`. Плюс `cache_control` на системные промпты
  студийных агентов. Плюс сторожок прогона получил признак завершения работы.
  В ВЕТКЕ: покадровые строки считают ЯЧЕЙКИ (`done/total`, зелёный по последнему
  кадру) вместо «зажигаться от первой» · строки перестали называть работников,
  которых нет (Casting и Музыка — ввод Директора, Continuity Critic назван по
  `EXEC-WCHK`, Key Art Critic удалён) · требования стадии сведены к ОДНОМУ
  источнику, из двадцати агентов сходился один; тест читает исходник раннера и не
  даёт разойтись снова · канон серии получил ось `ассет × стадия` и строку
  «Канон серии» первой в конвейере · канальный контур с правилом «канал
  ОПЦИОНАЛЕН» · плотность гэгов стала параметром эпизода и переживает раскадровку
  · воскрешён `sandy-gag-library` (был МЁРТВ с 14.06: скоуп `series_id` против
  UUID после миграции 0038) · плейбук раскадровщика стал `hard: true` (судья знал
  формулу, автор нет) · след в студии оставляется САМ из `gen-frame`/`gen-video`/
  `stitch`, а `stitch` пишет порядок сборки.
  РЕШЕНИЯ ДИРЕКТОРА 17–27 закрыты, включая пять противоречий: плотность считается
  КАДРАМИ (30 с → ~10 кадров → ≥10 различных, зазор ≤10 с, пик к 10–15-й секунде)
  · разнообразие у пакета тем, знакомость у ленты канала (3–5 эпизодов на движке)
  · хук = движение к 1-й секунде + сетап ко 2-й · плита `APPROVED` сразу, `LOCKED`
  после кадра с героем · `audience_kpi.md` — измерение, не гейт.
  Карта: `docs/doctrine/paradigm-roles-and-gates.md` (в ветке). Досье:
  `docs/topics/direct-calls-and-studio.md`.
Next:   **ПРОГОН ПО НОВОЙ ПАРАДИГМЕ СВЕЖЕЙ СЕССИЕЙ** — точка входа
  `docs/plans/paradigm-run-brief.md` (в ветке), работать в `C:\SandyStudio-nx`.
  Мера: сколько раз пришлось читать код (у E37 это стоило 40 минут из 108).
  Мерж в master — ТОЛЬКО после прогона. Осталось из UI: вкладка канала и
  отрисовка строки канона (данные роут уже отдаёт).
Mode:   ===5=== (Директор, день).
```

```
Date:   2026-08-05 (ЗРЯЧЕСТЬ СТУДИИ — день; после холодного прогона).
Status: **СТУДИЯ ВИДИТ ТО, ЧТО ПРОИЗВЕЛИ ПРЯМЫЕ ВЫЗОВЫ.** Директор открыл интерфейс и увидел
  по E36/E37 только строку эпизода: из семи инструментов прямого вызова строку `assets`
  заводил ровно ОДИН (`register-canon`), поэтому 195 кадров и 87 клипов трёх эпизодов
  существовали только файлами на диске. Причина — не баг, а выпавшее звено пути C
  («agent layer NOT called» решили; «студия ослепнет» — нет).
  Сделано: `_asset.ts` (`persistAsset` с `drive_path` — поле, без которого карточка молча
  пишет «no preview»), `register-media` (типы по конвенции `IMG-episode_ref` / `VID-shot-` /
  `VID-final_cut` / `AUD-music-main` + `shot_id` + длительность через ffprobe),
  `sync-episode` (раскадровка, аниматик, бриф и сценарий ИЗ ФАКТА прогона; сводит
  `budget_spent` с итемизированным леджером). E35 · E36 · E37 подняты целиком, канон-превью
  починено, D58 закрыт в `publish.ts`. Директор прогнал через студию весь блок дистрибуции:
  Online Editor → Publicist → Key Art → Distribution отработали штатно.
  ГЛАВНОЕ ЗНАНИЕ ДНЯ: **новая парадигма НЕ требует нового интерфейса.** Стадия красится по
  наличию APPROVED-ассета нужного ПРЕФИКСА (`pipeline-stages.ts:230`); задания нужны только
  для «крутится/упало». Витрине всё равно, кто выложил товар. Новый GUI = месяцы, конвенция
  + правка `ROW_DEFINITIONS` = день.
  Оплачено браком: стёр 10 правок таймингов Директора и его музыку — `persistAsset` заменял
  объект целиком. Восстановлено из его же ката детектором сцен. Правило: идемпотентность для
  машинного, ручное — только дополняется.
  Разрывы: гейт `EXEC-COPY` проверяет МЕНЬШЕ раннера (бриф) · события пишутся, до Директора
  не доходят («13 not today») · журнал правок хранит счётчик, не значения · `prod.log` мёртв
  с 31.07. Досье: `docs/topics/direct-calls-and-studio.md`. Разбор: шаги 15–19
  `docs/plans/cold-run-algorithm.md`.
Next:   НОВАЯ ПАРАДИГМА — ради неё сессия и начиналась. Первый шаг: отдельная ветка + второе
  дерево (вопросы 02/03 не отвечены: общая ли база, где стоит новое дерево). Реестр
  инструментария (`docs/plans/tools-registry-plan.md`) — фазы 1–3 в рабочем дереве, 4–5 на
  паузе. Канон-матрица Сэнди и красные D52/D53/D56 — прежние задания.
Mode:   ===5=== (Директор, день).
```

```
Date:   2026-08-05 (ХОЛОДНЫЙ ПРОГОН — SS-S15-E37 «Свежая краска»; ночь).
Status: ПАРАДИГМА ПОДТВЕРЖДЕНА ТРЕТЬИМ ПРОГОНОМ И ВТОРЫМ УМОМ. Эпизод проведён от темы до
  ката сессией с ЧИСТЫМ контекстом, только по `.claude/skills/ss-run`: 37,0 сек, 9 планов,
  9:16, $21,46 из потолка $25, 1 ч 48 мин, незапланированных касаний Директора — НОЛЬ,
  вопросов о том, что уже есть в маршруте — НОЛЬ. Лист приёмки (59 строк) написан ДО первой
  траты; плита локации отклонена за $0,018 вместо девяти кадров и девяти клипов.
  ГЛАВНОЕ ЗНАНИЕ ДНЯ: **стадия, которая ПЕРЕРИСОВЫВАЕТ вещь, не наследует правку.** Канон,
  исправленный только в промпте кадра, возвращался сломанным на клипе; тот же запрет в
  негативе промпта ВИДЕО держит. Доказано обратной пробой на одном кадре за $3,02 и объясняет
  разом три дефекта (зубы, пальцы, краска внутри стекла) — доктрина §12.
  Слепая приёмка (D52-обход): 13 отклонений по листу + 11 дефектов вне листа, из них один
  канон-дефект производитель не увидел вовсе. Прибор подтверждён вторым независимым умом.
  Разрывы маршрута D47-D57, три красных: blind-brief отдаёт приёмщику вердикты производителя;
  видеостадия не наследует канон-негативы; строки листа могут противоречить друг другу и
  матрица 7×7 этого не ловит.
  Доктрина обновлена: +16-бис (приобретённое состояние = канон), +16-тер (канон персонажа =
  МАТРИЦА проекций и состояний, не один рисунок), +21/22/23 (слепая приёмка · лист вперёд ·
  маршрут как продукт), уточнены §11 и §12, §VI закрыт вердиктом «готовность к смене
  доктрины: да».
Next:   задания Директора после показа — `docs/plans/cold-run-followup-tasks.md`:
  ДОДЕЛАТЬ канон Сэнди до канон-МАТРИЦЫ (все ракурсы + состояния, не менять существующее);
  системное правило «лицо или затылок по направлению движения»; механизм переноса
  приобретённого состояния между кадрами; разбор D47-D57, красные первыми. Публикация E37 —
  за Директором.
Mode:   ===5=== (унаследован; правки доков и прогон).
```

```
Date:   2026-08-04 (ПРОГОН ПО МАТРИЦЕ ВПЕРЁД — SS-S15-E36 «Магнит» собран; вечер).
Status: Эпизод СОБРАН: 38 сек, 9 кадров, вертикаль 9:16, штатная музыка Flacon Pop Loop
  с затуханием 2 с. $24,46 из потолка $50 (E35: $27,57 за 38 сек). Тема рождена
  КОНСТРУКТОРОМ ТЕМ (6 кандидатов по девяти осям), судил ОТДЕЛЬНЫЙ ум скиллом selection:
  A — Краска и Турникет, B — Зонт и Лента, C — Магнит, D — Фотобудка. Директор снял
  Турникет (движок уже снят в E31 — чего судья знать не мог: снятые эпизоды в банке не
  отмечены) и реабилитировал Магнит ключевой идеей: песок МАГНИТНЫЙ, поле действует
  ВНУТРЬ запечатанной колбы — единственная законная дверь в канон запечатанности.
  Лист ожиданий (11 строк на эпизод + 3-5 на кадр) написан ДО генерации; вопросов
  Директору за всю раскадровку — ОДИН.
  ГЛАВНОЕ ЗНАНИЕ ДНЯ: из четырёх «отклонений», предъявленных по строкам, Директор
  опроверг ТРИ. Н10 — строка обязана нести СИСТЕМУ КООРДИНАТ («вперёд» для лежащего —
  это на свои ноги, а не по горизонту). Н11 — строка не должна требовать невозможного
  при выбранной мизансцене (контур морды не читается, если морда обращена к герою).
  Н12 — оценочные слова («выраженный») возвращают вкусовщину. Н13 — строка, ради которой
  ломается канон, дороже того, что она защищает: кадр вычтен вместо четвёртой попытки,
  $0 вместо $1,51. Н14 — канон ПЕРСОНАЖА нужен отдельным чеклистом в КАЖДОМ кадре: его
  не было, и в финал уехал герой БЕЗ НОГ, которого заметил только Директор.
  МЕХАНИЗМЫ вместо обещаний: хук прогона получил вторую цель (`ss-run` растёт минимум
  раз в 5 ходов) и третью проверку «промпт новее листа приёмки = СТОП»; канон персонажа
  вписан чеклистом П1-П6; в `ss-run` добавлены путь C (7 инструментов + 4 огрызка с чужим
  episode_id), порядок работ по фактам E36 и три правила приёмки; инструменты больше не
  хардкодят эпизод — `RUN_EPISODE_ID` обязателен, без него отказ.
  СЛЕПАЯ ПРИЁМКА (долг прошлого прогона) ЗАКРЫТА, гипотеза ЖИВА: на E35 приёмщик с чистым
  контекстом нашёл 6 из 7 известных дефектов + 14 своих за 3 минуты и $0. Единственное
  ложное обвинение — из-за МОЕГО пересказа канона; отсюда `run/blind-brief.ts`, который
  собирает вход приёмщика из плит базы ДОСЛОВНО.
Next:   слепая приёмка E36 по новому брифу; затем упаковка и публикация — за Директором.
  Открыто: провайдер устойчиво НЕ умеет превращение песка внутри колбы (3 попытки, каждая
  ломала канон) — записано в маршрут; банк тем не помечает СНЯТОЕ, судья судит вслепую.
Mode:   ===5=== (Директор: «вперёд», 17:22).
```

```
Date:   2026-08-04 (ЭКСПЕРИМЕНТ ПО ТЗ — эпизод разобран покадрово диалогом; закрыт).
Status: E35 «Очередь» переставлен задним числом по матрице отклонений: 9 кадров,
  41 строка приёмки, ничего не генерировали. Журнал — docs/plans/spec-experiment-algorithm.md
  (21 шаг, писался по ходу под сторожком). ПРИБОРЫ: 1,7 вопроса на кадр (кривая 4→0,
  чистый повтор стоит 0 — платит ИЗМЕНЕНИЕ, не кадр); 11 из 14 ответов = клик по
  варианту с дефолтом, но ОБА самых ценных результата пришли свободным текстом →
  пульт = переключатели + одно обязательное свободное поле на кадр; брак вопросов
  4 из 19 (аппарат сырее пользователя); 4-5 мин на кадр. Граница легла сама: Директор —
  что должно быть видно / кто кому кто / ритм и разбиение; ум — доли кадра, секунды,
  ракурс, число опознаваемых. Аппарат ТРИЖДЫ породил строку приёмки сам (К3.4, освобождение
  места у окна, К9.4 по клетке ОБРАТНО). Два правила Директора внесены в spec-procedure.md:
  ИСХОД кадра обязателен (реакция исходом не является; сцепка исход N = вход N+1 только
  ВНУТРИ сцены — общий вид опровергнут Директором) и «В КАДРЕ НЕТ ВНУТРЕННЕГО» (уточнение
  сценария = обязательство кадру; мотив предметом и раньше решения; решение действием;
  без гротеска на лице). Находки Н1-Н5: единая форма ответа на экране · вопрос не пересекает
  границу кадра · названный риск обязан нести запасной ход · где варианты суть средства,
  спрашивать критерий · КАНОН МОЛЧИТ ПРО ОТНОШЕНИЯ персонажей (факт «Наковальня — друг
  Сэнди» жил только в голове Директора).
Next:   ОКУПАЕМОСТЬ НЕ ДОКАЗАНА — 7 из 7 задним числом это проверка на непротиворечивость.
  Доказательство даёт только эпизод, поставленный по матрице ВПЕРЁД, со счётом «сколько
  дефектов снято до траты». Долги: строка «отношение к главному герою» в плиты персонажей;
  колонка приёмки в раскадровку (ТЗ живёт там, отдельного документа нет); гипотеза СЛЕПОЙ
  ПРИЁМКИ с прошлого прогона по-прежнему не проверена.
Mode:   ===5=== (правки доков по ходу эксперимента).
```

```
Date:   2026-08-01 (ЧИСТЫЙ ПРОГОН — эпизод от темы до публикации прямыми вызовами; день).
Status: SS-S15-E35 «Очередь» ОПУБЛИКОВАН → https://youtu.be/iXlrtsPbpJg (public, 38,2 сек,
  8 кадров, музыка Flacon Pop Loop). Тема из банка `the_waiting`, замысел переписан
  Директором (один жест «я только спросить» повторяют четверо, Сэнди стоит ПЕРВЫМ,
  финал — успел за миг до грохота заслонки). Приборы: время 4 ч 21 мин общее, из них
  МОЕЙ работы ~51 мин (≈80 мин на 60 сек — в порог 120 УКЛАДЫВАЕТСЯ, промахнулся
  диалог, а не конвейер); генерация $27,57 из потолка $75 (97,6% — видео, 4 прогона
  в брак = $8,3); разум ≈$33; незапланированных касаний Директора 0.
  Записи прогона: docs/plans/clean-run-{brief,journal,algorithm,expectations-e35}.md.
  Алгоритм фабрики — 30 шагов, писался ПО ХОДУ под блокирующим сторожком (q6y).
  Разрывы D35-D44: гейт режима читает ===5=== из строки «Mode 1»; у прибора «деньги
  разума» нет носителя; банк тем не знает что снято; леджер не принимает трату мимо
  конвейера (FK job_id); 9:16 недостижим (clampSize); ПЯТЫЙ персонаж в кадре не
  опознаётся (потолок 4, в коде такого предела нет); музыки в фабрике нет вовсе
  (EXEC-MGEN = mock); обложку Short'а API поставить НЕ МОЖЕТ в принципе.
  Новое в репо: docs/doctrine/spec-procedure.md (матрица отклонений HAZOP×параметры —
  процедура ТЗ, 7 из 7 дефектов дня ложатся в клетки), скрижаль компоновки в
  eref-shot-composition, потолок опознаваемости в gpt-image-2-prompting,
  webapp/scripts/run/* (прямые вызовы: кадр, видео, канон, публикация, монтаж),
  хук run-algorithm-growth-guard.cjs.
Next:   ЭКСПЕРИМЕНТ ПО ТЗ (за Директором, после /clear): построить техзадание на эпизод
  ПОКАДРОВО методом «я задаю вопросы по клеткам матрицы — Директор отвечает», чтобы
  понять, какого рода ответы нужны и во что это превращается в интерфейсе (дропдауны
  против диалога). Точка входа: docs/doctrine/spec-procedure.md. Незакрытый пункт
  задания прогона — гипотеза СЛЕПОЙ ПРИЁМКИ, не проверена.
Mode:   ===5=== по факту работы (Директор: «поехали прогон», 13:38).
```

```
Date:   2026-07-31 (эксперимент «прямые вызовы» — эпизод доведён до финального ката; вечер).
Status: SS-S20-E01 СОБРАН: 37 секунд, 7 кадров, музыка + пульс сонара, гибридные переходы,
  финальная строка янтарём, уход в темноту и тишину. Директор поставил эксперимент —
  провести эпизод от середины до финала ПРЯМЫМИ вызовами, минуя приложение и Полину,
  чтобы сравнить с работой через конвейер. Результат: 4,5 часа, $52,40 по леджеру
  (видео $21 — 10 прогонов на 7 кадров, 3 дубля в брак), ~25 касаний Директора,
  ноль кликов в интерфейсе. Восемь разрывов инструмента записаны номерами D27-D34.
  Заведён канон SBL-object_inscription_panel — единственный источник фразы эпизода,
  вырезан из принятого кадра за $0. Новый скилл storyboard-shot-granularity.
  Метод дня и анализ хрупкости архитектуры (по тезису Директора о смене парадигмы) —
  docs/analysis/2026-07-31-direct-run-method-and-architecture-fragility.md
Next:   ЧИСТЫЙ ПРОГОН — эпизод от темы до публикации без агентов-посредников.
  Задание и приборы: docs/plans/clean-run-brief.md. Принципы, по которым его вести:
  docs/doctrine/content-factory-doctrine.md (читать ПЕРВЫМ). Прогон закрывает долг
  PLANET.md «производственная автономность не тестирована» и даёт числа для решения
  «переделывать студию или строить новое». От Директора нужны: серия и тема, потолок
  трат, решение о реальной публикации.
Mode:   Mode 1 — MANUAL. ===5=== выдан 29.07.
```

```
Date:   2026-07-29 (Прибор скорости на /factory — Тео, ===5===; вечер).
Status: Вторая таблица под per-episode: те же метрики, обрезанные окном
  (первый рендер EXEC-EREF → ПЕРВЫЙ EXEC-STITCH]. Директор: до первого стича —
  скорость, дальше украшательство. Верхняя таблица считала производству ре-стичи,
  тумбу, публикацию и собственный просмотр Директора; теперь скорость отдельно
  (E31 — 4.1 ч на 27 шотов, E34 — 0.8 ч на 12), трение — по-прежнему сверху.
  Churn/auto%/вердикты в новой таблице ОТСУТСТВУЮТ намеренно: скоркард считает их
  по версиям артефактов, а не по времени. Мерж `b633e95e`. tsc · 1630/1630 ·
  replay-pilot 30/30. НЕ задеплоено (ребилда не было — как и прежде).
Next:   за Директором: глянуть таблицу глазами после ближайшего ребилда.
