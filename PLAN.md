...
Date:   2026-07-17 (on-model гейт ПОСТРОЕН — Тео, КОД, ===5===). Реализован валидированный план целиком.
  ...
  **Живая валидация (п.6) ПРОГНАНА** ($1.85, 29 кадров, opus-4-8): fail-open доказан вживую (SH09 transient
  ...
  ложных → **E30 выставлен на medium (метаданные, merge)**. STRICT = {03,06,07,11,14,15,16,17,18,26,28} —
  строже 5-агентной калибровки и иначе судит прозрачность (совпал только SH06; SH01/04/08/12 пропустил, добавил
  03/07/11/17/18/28). SH25 калибровки детектор пропустил. Кадры {03,07,11,17,18,28} отправлены Директору на
  вердикт «реальный opaque-дрейф или перебор» — strict в прод только после его калибровки рубрики.
