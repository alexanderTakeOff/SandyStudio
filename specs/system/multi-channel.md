...
| 4e | Кеш/деньги/дефолты: фикс `mirroredCachePath` (SBL-* → `<S>/bible/media/`, legacy-flat шим) и `eref-upscale-only` (episodeCode из имени файла); `budget_log.series_id` (0055, BEFORE-INSERT триггер по паттерну 0049, бэкфилл; Полина `recordConciergeCost(seriesId)`; `/api/budget` скоуп в БД); publish-дефолты per-channel (`channels.metadata.publish_defaults` = category_id/made_for_kids/default_language → `publishDefaultsOf(passport)` → `uploadVideo` в EXEC-PUB + shorts-upload; UI в Settings→Channels, PATCH-whitelist; спека youtube.md приведена к коду); Storage-панель = 2 реальных поля — media cache dir (env `MEDIA_CACHE_DIR` через общий `persistEnvValue` из consent-флоу, mkdir-probe, без рестарта) + Drive root (`app_config storage.drive_root_name`, TTL-чтение в `driveRootName()` из persist-binary, валидация реальным `ensureFolder`); `project_root`/`media_storage_root`+проба H:\ удалены (гейты онбординга → `media_cache_writable`). Судьба `config/defaults.yaml` — открытый вопрос Директору | эта ветка (0055) |
