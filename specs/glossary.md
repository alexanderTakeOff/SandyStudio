...
11. [Shot staging / Постановка кадра](#11-shot-staging--постановка-кадра)
...
## 11. Shot staging / Постановка кадра

Словарь постановочного паспорта кадра (`storyboarder@v2`). Правила применения — скилл
`shot-staging`; здесь только канонические термины и их русские соответствия.

### Staging spec / Постановочный паспорт
Набор полей кадра, задающих ГДЕ стоит камера, КАКОЙ оптикой снимает, КАК расставлены
герои и ОТКУДА бьёт свет. Живёт полями шота в раскадровке (решение Директора 2026-08-25,
вопрос 14 · вариант 1), а не отдельным документом: кадр — единица истины, паспорт — его
свойства. Числа хранятся для проверки и воспроизводимости, в промпт уходит их словесный
эквивалент («лук»): генеративные модели не считают миллиметры.

### Shot size / Крупность
Размер плана по линии реза на теле. Канонический ряд: `ECU` деталь · `BCU` плотный
крупный · `CU` крупный · `MCU` средне-крупный (грудной) · `MS` средний (поясной) ·
`MLS` средне-общий (американский) · `FS` в рост · `LS` общий · `ELS` дальний.
**Не путать с ракурсом** — это независимая ось (до 2026-08-25 обе жили в одном поле
`camera_angle`, отчего «крупный план снизу» был невыразим).

### Crop line / Линия реза
Анатомический ориентир, по которому проходит нижняя кромка кадра. Резать разрешено
МЕЖДУ суставами (`mid_chest`, `mid_thigh`, `mid_forearm`); рез ПО суставу — запястье,
локоть, колено, щиколотка, шея, подбородок, талия — читается как ампутация и запрещён.

### Subject distance / Дистанция до объекта
Расстояние камера→герой в метрах. **Единственный параметр, задающий перспективу.**
Крупность без дистанции — недоопределённый кадр: один и тот же средне-крупный план с
0,5 м и с 2 м это два разных лица (с 40–60 см основание носа шире примерно на 30%).

### Lens look / Лук объектива
Категория оптики, а не число: `ultrawide` · `wide` · `normal` · `portrait_tele` ·
`tele` · `macro`. Эквивалент в миллиметрах (`lens_equiv_mm`) хранится для валидации;
в промпт уходит лук со своим признаком («85mm portrait lens, natural face proportions,
no wide-angle distortion»). Ни один провайдер не документирует физическую трактовку
миллиметров и диафрагм — модель воспроизводит статистику подписей, а не оптику.

### Rig / Риг
Чем держится камера: `tripod` · `handheld` · `gimbal` · `crane` · `drone` · `selfie` ·
`selfie_stick` · `pov` · `body_cam`. Риг — не движение, а связка «дистанция + оптика +
характер тряски». Селфи возможно ТОЛЬКО при `rig=selfie*` и дистанции ≤1,2 м; отсюда
имя дефекта — **«селфи с трёх метров»**: заказан селфи-лук на портретной дистанции.

### Camera height / Высота камеры · Camera angle / Ракурс
Две независимые величины: высота точки (`ground` · `hip` · `chest` · `eye_level` ·
`high` · `overhead`) и наклон оптической оси в градусах (<0 — снизу вверх, >0 — сверху
вниз). Камера на уровне бедра с горизонтальной осью — НЕ нижний ракурс.
`dutch_deg` — завал горизонта, третья независимая величина.

### Blocking / Блокинг · Screen direction / Экранное направление
Где герой в кадре (`subject_position` по сетке третей), куда смотрит
(`looking_direction`: `screen_left` / `screen_right`), сколько воздуха оставлено по
взгляду (**looking room / воздух по взгляду**) и над головой (**headroom / воздух над
головой**), и как распределены планы глубины (`depth_layers`: передний · средний ·
дальний).

### Axis of action / Ось действия (правило 180°)
Воображаемая линия между героями; камера остаётся с одной её стороны, иначе экранные
лево/право переворачиваются. Поле `axis_side` постоянно внутри сцены. **Модели этого не
держат в принципе** — каждый кадр генерируется независимо; соблюдение оси обеспечивается
снаружи, повторением экранного направления в промпте КАЖДОГО кадра.

### Key / Fill / Back light / Рисующий · Заполняющий · Контровой
Три функции приборов. Рисующий задаёт форму и тени, заполняющий поднимает тень (контраст),
контровой отделяет фигуру от фона. Отсутствие контрового = фигура «влипла» в фон;
рисующий на оси камеры (`on_axis`) = плоское блёклое лицо — самый частый брак света.

### Light pattern / Схема света
Именованный рисунок тени на лице: `butterfly` (бабочка) · `loop` (петля) · `rembrandt`
(треугольник света на затенённой щеке) · `split` (ровно половина лица в тени).
Дополняется стилем наложения: `short` (освещена дальняя щека) / `broad` (ближняя).
В промпт схема пишется вместе с признаком, по которому её видно.

### Contrast / Контраст (вместо lighting ratio)
Числовое световое отношение (4:1) модели игнорируют — контраст задаётся словом:
`high` · `medium` · `flat`. Так же и температура: не кельвины, а `tungsten_warm` /
`daylight_cool` / `mixed`.

### DOF / Глубина резкости · Focus subject / Субъект фокуса
`shallow` · `medium` · `deep`. При малой глубине обязателен `focus_subject` — на чём
именно резкость («её глаза»), иначе резкость уезжает не туда. Перевод фокуса
(**rack focus / перевод фокуса**) описывается как событие: с чего, на что, в какой момент.

### Camera move / Движение камеры
Ровно ОДНО на кадр: `static` · `pan` · `tilt` · `dolly` · `truck` · `pedestal` · `arc` ·
`crane` · `whip` · `tracking`. Два движения в одном промпте дают спутанный результат;
облёт больше ~30° за 5 секунд даёт варпинг. Отличать `dolly` (камера едет, перспектива
меняется) от `zoom` (меняется фокусное, перспектива та же).
---
