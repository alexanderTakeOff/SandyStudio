...
## specs/production/audience_kpi.md | v0.2 | ИЗМЕРЕНИЕ, НЕ ГЕЙТ
...
## СТАТУС (решение Директора 27, 2026-08-06)

**Числа этого документа — ПРИБОР, а не условие пропуска.** До 06.08 они не были
подключены никуда: ни `gag_rate`, ни `hook_strength`, ни проверки `CHK-K0*` не
читались ни одной строкой кода, а секции в конфиге, на которую ссылается спека,
не существует вовсе. Документ выглядел действующим и не действовал.

**Почему не гейт.** Плоский счёт гэгов геймится за минуту — тем же гэгом в новом
костюме. Условием пропуска служат ГРУППЫ механизмов
(`series-episode-theme-selection`) и покадровый план плотности (`resolveGagPlan`,
выводится из хронометража эпизода). Число остаётся наблюдением: увидел «1,2 гэга
в минуту» — знаешь, что эпизод пустой, но никого этим не блокируешь.

**Что подключено (06.08):** `sync-episode` печатает по факту число кадров, средний
кадр, кадров в минуту, предупреждает о кадрах длиннее 10 секунд и сравнивает с
ожидаемым при этом хронометраже. Не блокирует ничего.

**Что остаётся неподключённым:** `philosophy_density`, `hook_strength`,
`recognition_moments` и блокирующие `CHK-K0*`. Читать как намерение, не как
работающий механизм.

**`gag_rate` ниже устарел ПО ЕДИНИЦЕ ИЗМЕРЕНИЯ.** «2–6 в минуту» на ролике в
30–45 секунд разрешает один гэг на весь ролик. Действующая единица — КАДР:
каждый кадр несёт свой гэг, 30 с → ~10 кадров → ≥10 различных.
