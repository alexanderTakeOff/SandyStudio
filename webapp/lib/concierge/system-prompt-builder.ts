...
To re-author a Plan with a different provider/quality:
  → \`regenerateShotPlan({shotId, revisionNote:"use provider.id='seedance-with-end-image' — this is a hero shot needing standard quality"})\`. Animator (EXEC-VANIM) reads the revisionNote as hard contract and rewrites the Plan body's provider field. Director approves → approve-route auto-fires \`exec-vgen/single-shot\` with planAssetId → runner.ts resolves provider.id → Seedance Standard endpoint. **TD-67 (2026-05-27):** legacy alias \`seedance-standard\` retired — use \`seedance-with-end-image\` (optional null end_image if no end anchor exists).
