// ──────────────────────────────────────────────────────────────────────────────
// lib/agents/audience-metrics.ts
// ONE assembly of a channel's VideoMetric[] + shorts→episode funnel from live
// YouTube APIs + the archived reach CSVs. Extracted from app/api/audience/route.ts
// (multi-channel Phase 4b) so the Audience dashboard AND the daily advice persist
// (inngest hog-report-poll) read the same numbers — two assemblies would drift.
//
// Read-only against YouTube; auth is ALWAYS the caller-resolved channel token
// (no global-token fallback — multi-channel §3).
// ──────────────────────────────────────────────────────────────────────────────

import type { SupabaseClient } from '@supabase/supabase-js';
import type { Database } from '../supabase/types.gen';
import { listAllUploads } from './providers/youtube';
import {
  getVideoStatistics,
  getVideoAnalytics,
  getRetentionCurve,
  isCompletionReadable,
} from './providers/youtube-stats';
import { readReachMetricsFromArchive } from './providers/youtube-reporting';
import type { VideoMetric } from './analytics-advisor';
...
/**
 * Assemble every video's metric row for ONE channel: live counters + analytics
 * (90-day window) + retention curves (public long-forms) + archived reach,
 * plus the shorts→episode funnel from the distribution ledger
 * (episodes.metadata of THIS channel's series only).
 */
export async function assembleChannelAudience(
  ...
  const { data: chSeries } = await supabase.from('series').select('id').eq('channel_id', channelId);
  ...
  const reachById = await readReachMetricsFromArchive(supabase, channelId).catch(
...
/**
 * The channel's gag/category space for the advisor's holes map, from data:
 * union of `series.metadata.gag_taxonomy[]` across the channel's series.
 * Empty when no series declares one — the advisor then simply emits no holes
 * (honest silence, never another series' taxonomy).
 */
export async function resolveChannelTaxonomy(
  supabase: SupabaseClient<Database>,
  channelId: string,
): Promise<string[]> {
  const { data } = await supabase.from('series').select('metadata').eq('channel_id', channelId);
  const out: string[] = [];
  for (const row of (data ?? []) as Array<{ metadata: unknown }>) {
    const meta = row.metadata && typeof row.metadata === 'object' && !Array.isArray(row.metadata)
      ? (row.metadata as Record<string, unknown>)
      : {};
    const tax = meta.gag_taxonomy;
    if (Array.isArray(tax)) {
      for (const t of tax) if (typeof t === 'string' && t.trim() && !out.includes(t)) out.push(t);
    }
  }
  return out;
}
