// ──────────────────────────────────────────────────────────────────────────────
// lib/providers/anthropic-text.ts
 ...
 *
 * Lives here since Ф1 (2026-08-07): it is read only by this provider, and a
 * provider must not depend on the agent layer it is meant to outlive.
 */
...
const MODEL_RATES: ReadonlyArray<{ prefix: string; rate: ModelRate }> = [
  // ВНИМАНИЕ на порядок: современные Opus (4.7 и 4.8) стоят $5/$25 — ВТРОЕ
  // дешевле прежних Opus-4.x ($15/$75), поэтому их префиксы обязаны стоять
  // ПЕРЕД общим `claude-opus-4`. Иначе `claude-opus-4-8-1m` (модель единого ума)
  // матчится на старую строку и счёт завышается втрое. Найдено 2026-08-08 сразу
  // после того, как D81 включил запись стоимости ума в budget_log: цифра пошла
  // бы в журнал неверной с первого же хода.
  // 4.7 и 4.8 тарифицируются ОДИНАКОВО (различие только в fast-режиме, который
  // мы не используем: $10/$50 у 4.8 против $30/$150 у 4.7).
  { prefix: 'claude-opus-4-8',  rate: { inputUsdPerMillion: 5.00, outputUsdPerMillion: 25.00 } },
  { prefix: 'claude-opus-4-7',  rate: { inputUsdPerMillion: 5.00, outputUsdPerMillion: 25.00 } },
