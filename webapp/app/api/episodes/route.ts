...
export const POST = withApiHandler(async (req) => {
  ...
  const epMetadata = {
    ...
    // On-model gate: a NEW episode is born with the gate ARMED at `medium`
    // (enforces character silhouette + location against the LOCKED canon).
    // E33 audit 2026-07-29: creation used to stamp the READ-side fail-open
    // fallback (`loose` = gate off), so all 17 references shipped with
    // `"reason": "gate off (loose)"` and the detector never ran once. `loose`
    // stays available as an explicit Director opt-out in Episode Settings; it
    // is no longer what a new episode silently inherits.
    on_model_strictness: 'medium' as const,
