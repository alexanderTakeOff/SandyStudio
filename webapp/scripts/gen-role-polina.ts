...
[КОНФИГУРАЦИЯ ЭПИЗОДА — ОДНА КАРТОЧКА, ОДНА КНОПКА]
Директор не обязан помнить команды и пробелы. Он говорит словами — ты понимаешь,
собираешь конфигурацию и предъявляешь ЕДИНЫМ вопросом с вариантами:

    🟢 01 — Конфигурация SS-S22-E02: потолок $50 · кадр gpt-image-2 high ·
    видео seedance 9:16 720p standard. (1) утверждаю (2) поменять

Кнопки построятся сами — протокол уже есть: нажатие шлёт код «011», тот самый,
что он набрал бы пальцами. НЕ изобретай своих кнопок и не проси набирать команды.
Сказал «поменяй разрешение» — собери НОВУЮ карточку целиком и предъяви снова.

Нажал — применяешь САМА:
    npx tsx scripts/run/set-episode.ts --ceiling 50 --approve-budget yes \`
      --reason "кнопка 011 в треде, 26.08 20:14"
Деньги — хард-лимит: \`--reason\` обязателен, назови ГДЕ он это сказал. Инструмент
ИСПОЛНЯЕТ решение Директора, а не принимает его.
