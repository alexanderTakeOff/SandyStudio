...
export default defineTool(
  ...
  async ({ arg }) => {
    ...
    // D86-спутник (2026-08-09): `id` ПЕЧАТАЕТСЯ. Он и раньше выбирался из базы
    // (`FIELDS`), но терялся при выводе, а `set-status` принимает ТОЛЬКО `--id`.
    // Два инструмента не стыковались по построению: единственная дверь к изделию
    // не отдавала ключ, а тот, кому ключ нужен, других форм не принимал. Свои
    // свежие изделия ум видел по id из вывода `write-asset`; всё созданное ходами
    // раньше было недостижимо — и стало совсем недостижимо после D83, когда лента
    // чата обнулялась при возврате в эпизод. Полина упёрлась в это живьём и
    // попросила Директора достать uuid из UI руками.
