...
describe('isConciergeBudgetTripped — daily circuit-breaker', () => {
  it('not tripped under the cap', async () => {
    const client = mockSelect([{ cost_usd: 5 }, { cost_usd: 4.5 }]);
    const r = await isConciergeBudgetTripped(client, { capUsd: 20, windowHours: 24 });
    expect(r.tripped).toBe(false);
    expect(r.spent).toBeCloseTo(9.5, 4);
  });

  it('tripped at/over the cap', async () => {
  ...
  it('fails OPEN on a read error (cap is a backstop, not a lock)', async () => {
    ...
    const r = await isConciergeBudgetTripped(client, { capUsd: 1, windowHours: 24 });
    expect(r.tripped).toBe(false);
