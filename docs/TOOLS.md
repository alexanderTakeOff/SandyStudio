...
| [`peek-canon`](#peek-canon) | Показывает плиту канона по слагу: текст дословно и картинку, если она есть. |
...
Показывает плиту канона по слагу: текст дословно и картинку, если она есть.

```
npx tsx scripts/run/peek-canon.ts --slug <slug> [--out <out>] [--chars <chars>]
```

| аргумент | обязателен | по умолчанию | допустимо | что это |
|---|---|---|---|---|
| `--slug` | да | — | — | слаг канона без префикса `SBL-` |
