...
| `--type` | — | `` | — | ТОЧНОЕ значение `file_type` в базе (не голый код из CLAUDE.md §3 — там таблица типов, а не имён столбца): `SCR-script`, `STB-storyboard`, `SPC-brief`, `SPC-ref_plan-<shot>`, `SPC-shot_plan-<shot>`, `IMG-episode_ref…`, `VID-final_cut`. Берётся самое свежее. Отказ «не найдено» — не значит «не существует»: возможно, тип назван неточно; проверь listSeriesBibles/getEpisode или спроси точный `file_type`. |
